...
⭐⭐⭐ **pair 也走完了三层，含两次当天的自我更正**（`lurkswap --focus i --partner j`，三臂 + placebo + **D 臂**，
另有**四条独立谱系**的复现集 d2s-pop48 / d2s-eval15 @99）：
① ⭐⭐ **「它还在」不是归因，「搬家到现在持有那个标签的**别的** agent」才是**（坑 #37）——
两条臂各自只拆一半配对，所以留存正是「温和退化」的预测；⚠ 留存型证据又干净又稳，比真证据还漂亮。
② ⛔ **arm C 曾有真 bug**（helper 默认参数把换点钉死在 slot 0 ⇒ `--focus≠0` 的**五次**运行换错身体；
落地断言拿自己 re-derive 的地图比对、容差 8m 架在 4m 间距上，**恒真**）——坑 #38，已修并全部重跑。
**修完的九个冠军**：**五个正向归因，全部归给出生位置**（C 臂 by spawn 38–78% vs by carrier 3–9%）；
三个归因不了（两个被自己的 placebo 判掉）；**s1 红 1-3 不是位置、身份证据弱且散（28% ± 20.8）⇒ 未确立**。
⭐ **D 臂（整份配对一起搬）如预期**：搭档跟着「捆」走 ⇒ 两臂都没动的**感知噪声键**什么都没扛 ⇒ **双标签分解成立**。
⇒ **lurk 与 pair 指向同一件事：此处一切可归因的结构都是几何的**，⭐ 对 SUBSTRATE 的 Club/Player-block 仍是降温读数。
⭐⭐ **那条悬案当天也收掉了**（`runs/g2-pair-identity-predictions.txt`）：加了**搬家基线**（「搬到 k」要对照
「(k, 搭档) 本来就有多少」）和**对称检验**（改另一半的身份）。**对称检验通过**：one-hot 3 给谁、谁就和读 one-hot 1 的身体配对，
三次全中且全高于基线（60/2 · 73/32 · 27/2），配合 by-spawn 全 ≤ 2% ⇒ **299 代那一对跟身份走**。
⚠ 不对称是真限制：**锚（身份 1）只转得动相邻身体，搭档（身份 3）给谁都转得动**。
⛔ **但世代上不稳**：1-3 更早根本不存在（150 代 3-4、200 代 1-2、250 代 3-4，全相邻、且都跟位置走）
⇒ **它是 299 代那个基因组的性质，不是谱系的性质**。
⭐⭐⭐ **G2 这一轮的总结论 = 这个对比：位置型角色（lurk 0 号）稳了 227 代；身份型搭档只出现过一次、很晚、不稳定。**
⛔ 引用身份那条必须带「仅 299 代」+「锚/搭档不对称」。
**下一步**：G2 六个词条已全部结案，⭐ 主线可以进 **G3（style space / lineage）**；
留在 G2 的小尾巴只剩 s2 蓝那个「只游荡不开火」的形状（没做 birth/干预）。
