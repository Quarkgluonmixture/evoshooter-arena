...
⇒ 按 claim ladder 只到 **CANDIDATE**，⛔ 不是「进化出了 lurker」。**下一步（各需新预注册，先做哪个是一次选择）**：
① **birth** —— 同一 detector 扫 s1 世代，找 0 号这个模式什么时候出现（便宜、纯描述）；
② **intervention** —— 把 slot one-hot 在 0 号与队友之间对调：五个身体共用一个脑子，角色只可能住在 slot 输入里，
看形状跟 **slot** 走还是跟**身体**走（这条才是 VISION §12.1 要的那一级）。
