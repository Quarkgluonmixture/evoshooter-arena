...
⛔⛔ **同日的外部效度检验给整个 G3 划了界：「哪几代动了」是地图特有的。** 同一网格跑三张图，一致率 42% / 83% / 43%
（偶然水平 50% / 62% / 50%）⇒ **三对里两对不高于偶然**；唯一跨图成立的是「最早那一步动得最凶」。
⚠ 连 `det` 也是地图特有：s1 红 gen 160 在训练图 own-noise 0.08、换图 1.27/1.45（**在自己长大的图上不接敌，换图就打**）。
⇒ **chunk 1 的每句风格结论必须带「在训练图上」**；⛔ **chunk 2 不得拿这条轨迹讲「时代」**。
⭐ 用词纠正：换地图不是「把噪声底做强」（地图/出生点/策略都确定，本就没多少采样噪声），而是**外部效度**检验。
⭐⭐ **chunk 2a（eras）当天也做了 = `npm run eras`**：⛔ 没造新矩阵 —— `crossplay` 已有 `--gens every:N`/`--maps`/
双色对打/接触分母/非传递环，且 `crossplayFile.ts` 明令下游不得重算 ⇒ 新脚本只**消费** `rowMean/balanced/sight/cycles`
（读到旧格式文件直接**拒绝**，不偷偷补算）。
**结果**：进步 Spearman(代数, rowMean) —— 蓝 **0.94/0.94/0.94**（三图全单调）· 红 0.83/0.43/**0.03**；
跨图 Kendall tau（全 12 entrant）0.52/0.64/0.39 ⇒ **冻结的闸没过 ⇒ ⛔ 本轮不授权任何「时代」结论**。
⚠ 事后逐色（⛔ 不算过闸）：蓝 0.73/**1.00**/0.73、红 0.47/0.20/0.47 ⇒ **蓝的阶梯换图完好，红的是训练图产物**。
非传递环按图 **0 → 1 → 22**：离开训练图非传递性爆炸。
⇒ ✅ 本轮唯一被授权的是**方法结论**：**「进步」主张必须逐色做、且要在训练图之外复核。**
**下一步**：chunk 2b = lineage/血统追踪（⛔ 通过**逐色**闸之前只做图内比较）· chunk 3 = 观战侧展示面。
