...
⛔⛔ **2026-09-15 收口检验：lurk 角色不可跨图搬运** —— 三张图**出生点/点位逐位相同、只有掩体不同**，而角色跟出生位走，
即便如此：s1 红 @299 图 11 上**还在**（0 号 60%、2.20×）、图 23 上**整个塌掉**（0 号 **0%**、1.10× = 随机）；
反向也成立：**s2 红训练图上没有、图 11 上长出来一个**（0 号 64%、2.76×）。
⇒ 角色是 **(基因组, 出生位, 这张图)** 的联合性质；⛔ **下面每一条 lurk 结论（含 227 代）都只在训练图上成立**；
⛔「这支队有个 lurker」不是队的性质。机制（对照/零假设/birth/干预）都没被推翻，只是**都跑在一张图上**。
⭐⭐ **pair 已跨图复核（同日）**：**十二格里每格都存在互选搭档、且全高于各自零假设**（1.49–2.56×），
⛔ 但**是哪一对**四个冠军里三个换图就变（s1 红 1-3 → 2-3 → 1-4）。
⇒ ✅「这支队会保持一对搭档」**跨图成立**（G2 第一条扛住换图的陈述）；⛔「搭档是 1 号和 3 号」要带地图限定。
✅ **pair 的归因也跨图复核了**（`lurkswap --map`，每张图取那张图自己的最强对）：**两张离图上都复现** ——
搭档搬到**接手位置**的身体上、从不搬到接手身份的（C 臂 by spawn 70/46/20% vs by carrier 4/9/1%），
在**图 23**（搭档换成 2-4、且 lurk 角色在那塌掉）**依然成立**。⚠ 效应量离图变弱、placebo 也一起降 ⇒ ⛔ 跨图不比大小，只认方向。
⇒ ⭐⭐ **补完的正是缺的那块：机制会迁移，身份不会。**

⭐⭐⭐ **全天统一规律（最该带走的一条）：结构的「总量层」跨图稳，「身份层」不稳。**
style：有几代在动稳 / 哪几代动不稳 · eras：越晚越强部分稳 / 逐代次序不稳 · pair：有搭档稳 / 是哪一对不稳 ·
lurk：连存在性都不稳（它是**单个身体的路线**，而路线由掩体决定）。
⇒ ⛔ **不带地图限定就不要点名任何个体**（哪个 slot／哪一对／哪一代）；✅ 关于「结构有多少」的陈述可以搬运。
...
**#40** ⭐⭐ **一张图上的行为结论默认不跨图**（哪怕出生点逐位相同；本程序的头条就栽在这）；
**#39** ⭐ **< 8 个点的秩统计量只是线索不是证据** —— 同一天它骗了我两次（tau 1.00 与 Spearman 0.83 都在加密采样后垮掉）。

⭐⭐ **G3 进行中：chunk 1 / 2a / 2b 已交付**（数字与经过在 `docs/ROADMAP.md` G3 节 + `LOG.md`，预注册在 `runs/g3-*.txt`）：
`npm run style`（逐代风格向量 + 噪声底）· `npm run eras`（消费 crossplay 产物读进步/跨图序稳定）· `npm run lineage`（从基因组推断血统）。

⛔⛔ **三条硬约束，下一个 session 必须带着走**：
1. **风格轨迹是地图特有的**：三张图上「哪几代动了」一致率 42/83/43% vs 偶然 50/62/50% ⇒ 每句话都要带「**在训练图上**」。
2. ⛔ **「时代」叙事已删除**：逐色闸在加密采样 + 独立谱系下复测，**四条轨迹全 FAIL**；
   「蓝方特殊」是 6 个采样点的运气（tau 0.73/1.00/0.73 → 0.60/0.59/0.48），红方从反方向同样翻车（Spearman 0.83 → 0.12）。
   ⇒ 只剩方法结论：**进步要逐色读，且只有粗趋势曾看似跨图稳，逐代细次序四条轨迹一条都没迁移过**。
   ⚠ 未排除的替代解释：离图后无接触格子分布不均（蓝 15–24 格 vs 红 3–4 格）⇒ 修法是**离图加大每格场次**，⛔ 不是降门槛。
3. ⭐⭐ **没有任何一次「换人」**：冠军基因组是**受约束的随机游走**（lag 曲线 4.3–4.6×：远高于平、远低于 √lag）；
