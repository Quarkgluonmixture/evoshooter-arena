...
⭐⭐ **pair 的读数留下了下一个问题**：四个冠军都有稳定互选搭档，但**两个红冠军跳过中间那个人配对**（1-3 / 0-2，出生间距 8m），
而两个蓝冠军配相邻（0-1，4m）。干预刚说角色跟位置走 ⇒ 这是**第一个出生几何解释不了的形状**（⚠ 每色 n=2，是模式不是结论）。
⇒ **下一步（需单独预注册）= 把 `lurkswap` 三臂套到 pair 统计上**：搭档跟两个 one-hot 走还是跟两个位置走？
