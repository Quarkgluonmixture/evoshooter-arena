...
2. **「进步」必须逐色说、且要出训练图复核**：Spearman(代数, rowMean) 蓝 **0.94/0.94/0.94**、红 0.83/0.43/**0.03**；
   跨图 Kendall tau 全场 0.52/0.64/0.39 ⇒ **冻结的闸没过 ⇒ ⛔ 本轮不授权任何「时代」结论**；
   事后逐色（⛔ 不算过闸）蓝 0.73/1.00/0.73、红 0.47/0.20/0.47 ⇒ **蓝的阶梯换图完好，红的是训练图产物**。
   ⚠ 非传递环按图 **0 → 1 → 22**：离开训练图非传递性爆炸。
**下一步**：chunk 2b = lineage/血统追踪（⛔ 过**逐色**闸之前只做图内比较）· chunk 3 = 观战侧展示面。
