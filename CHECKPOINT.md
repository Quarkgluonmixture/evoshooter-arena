...
**C2 进行中（2026-09-13）**：死亡坐标已关闭（探针 `A1-P24`）· 公开目标状态已进观测（探针 `A1-P25`）。
⭐ obsDim 按模式给：**koth 仍是 100 / genome 5324**（回合状态字段只在 capture 模式发放 ——
无条件发的那一版 koth 下三个字段恒 0，实测把 kills 打到 0.35/1.65、首枪拖到 34 s）。
capture 单点位 103 / 双点位 109。⛔ **capture / 双点位下的 genome 与 koth 不互通**。
⚠⚠ 新坑 **#27**：**改了 genome 长度，「同 seed A/B」就不是配对实验** —— 初始种群整批重抽。
⭐ 并且量到了噪声上界：只把 100 个同内容字段**重排顺序**，2 个 seed 上就能造出和真效应同量级的差
⇒ ⛔ 2-seed 行为侧 A/B 只读方向。
**C2 还剩**：explicit round lifecycle · kill feed 作为可被观战消费的公开事件源 ·
「死亡玩家不得继续给活人 radio truth」的规则化（⚠ comm/血量已清零，但还没有探针守）。

**（历史）下一刀 = Phase C2（公开回合状态）。** 现在挡路的是**进化 agent 看不见回合状态**（点位在哪 / 哪个 armed /
