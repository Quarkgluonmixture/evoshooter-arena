...
## Current cursor（2026-09-19）

⭐⭐ **E2b 结案，一句话**：**这个世界允许一支队「反应」，但不允许它「不可预测」。**
E2 的 demand 表是本仓唯一读正的一条（守方均衡混合 68/32，比任何单一打法的最坏情况高 14.5pp）。
`npm run mixing` 反过来问供给侧：同一个对手下，**四个冠军 × 两图 × 攻防的计划熵恒为 0.000 bit**
（世界挂 0.904）。机制在源码里：rng 只滚命中骰子、感知抖动是纯哈希、出生点固定 ⇒ **玩家没有私有骰子**。
而对照 `ctl alternating` 在**同一世界同一仪器**上读 **1.000** ⇒ **世界装得下混合，只是不会为
「每回合输入都相同」的策略生出一个。** ⇒ 本仓第一次把 demand 判成**收不到**而非**没找到**。
⛔ **它不授权任何施工**（私有骰子 / 随机出生点 / 多回合历史各是独立杠杆，且混合只对会适应的对手兑现）。
逐条数字在 `docs/ROADMAP.md` Phase E2b + LOG 2026-09-19，预注册在 `predictions/e2b-mixing-predictions.txt`。
...
| `npm run mixing -- <runs>` | ⭐ **回合计划变不变**（E2b 供给侧）：t=T 时五个 slot 最近点位的 5 元组熵，三条臂 own dice / opponents / maps，熵只在单格内算 | ⛔ 0 bit 不说明冠军弱；⛔ `ctl alternating` 不读 1.000 就别读冠军行（坑 #42）；⚠ alive5 < 50% 的格子已被摘要行排除 —— 那是伤亡名单不是计划 |
