...
## Current cursor
**A3.2 — 位置换成感知（bearing + range cue + quality）+ 视距连续化**。
2026-09-11 一晚关掉四刀：A0 22:15 · A1 22:30 · A2 22:55 · **A3.1 23:00**。

- A1 给了仪器：`npm run leaks` 打印当期信息泄漏矩阵，`tests/leak.test.ts` 断言「实测 == 登记的精确字段集」。
  ⇒ **每一刀的验收就是看哪几条 probe 该翻绿**，别再靠读代码自证。
- A2：敌情私有化（`contact[player][enemy]`）。A3.1：删掉 `enemy*.hp` / `exposureToMe` / `facingDot`，obsDim **100 → 91**、genome **4964**。
- ⚠ **三条别踩**：① `coverRatio` 不可跨 A2 比较（尺子变了，GOTCHAS #12）；
  ② 零和指标（kills/deaths/damage）不能当独立 cell 写预测（#14）；
  ③ bench 前先看 `uptime`，忙机器会让 ms/match 虚高 35%（#15）。
- ⚠ **ladder 追踪**：末代冠军对第 0 代两个方向都 ≥50% 的血统数 = baseline 4/4 → A2 3/6 → A3.1 3/6。
  连续两刀同一水平 ⇒ 更像预算/尺子问题。下一步不是加 seed，是把 TODO 里的 **cross-play 矩阵**做出来。

下一步严格按 `docs/ROADMAP.md` Current Cursor 的 A3.2；之后才是 A3.3（front-biased 几何）。
