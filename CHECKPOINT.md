...
**四个已结案，没有一个读到形状**：**trade**（条件化零假设下 lift 0.78–0.82 ≈ 随机）· **crossfire**（第二版 precision 过了，
冠军**一次都没形成**，中位夹角 3–22°）· **rotate**（precision 两轮都没过：它量的是「多久跨一次中线」不是「响应压力」，冠军未读）·
**tempo / man-disadvantage**（2026-09-15，precision 没过，冠军未读）。
...
同时让负对照**空过**。⇒ 下一个 detector 选形状时优先挑**零假设不依赖回合时钟**的（lurk / 孤立路径、pair coordination 这类**空间**形状，
它们的置换可以在场内做）；真要问 tempo 得换**场内分叉 + 干预**（G4c），那是新预注册。
...
| `npm run detect -- <runs> --gen G` | ⭐ **只读战术 detector**（G2）：trade（条件化零假设）· crossfire（夹角 + 最小距离）· rotate ⛔已关 · tempo ⛔已关（逐玩家配对 DiD + `donor agree/∩` 列）；每个都自带手写正/负对照，`--precision` 只跑对照 | ⛔ 形状不是意图；⛔ 结果永不进训练（T8 防火墙）；⛔ precision 没过就别读冠军；⛔ 已关的两段不打印冠军行（`--reopen` 需配新预注册） |
