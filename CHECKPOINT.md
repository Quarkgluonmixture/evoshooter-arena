...
⭐⭐ **G3 进行中，今天交付两块**（数字与经过在 `docs/ROADMAP.md` G3 节 + `LOG.md`，预注册在 `runs/g3-*-predictions.txt`）：
- **chunk 1 = `npm run style`**：逐代风格向量（复用 `match.ts` 的 22 个 `METRIC_KEYS`，⛔ 不新造特征）+ PCA 2D + 逐代噪声底，
  对手固定为另一色最终代。两道闸都是防我自己读错加的：不等间距扫描会警告；`det` 行（own-noise<0.3）不得读。
- **chunk 2a = `npm run eras`**：⛔ 没造新矩阵，只**消费** `crossplay --out` 的 `rowMean/balanced/sight/cycles`（旧格式直接拒绝）。

⛔⛔ **这两块各自撞出一条硬约束，下一个 session 必须带着它们走**：
1. **风格轨迹是地图特有的**：同一网格三张图，「哪几代动了」一致率 42/83/43% vs 偶然 50/62/50%（两对不高于偶然）
   ⇒ chunk 1 的每句话都要带「**在训练图上**」；连 `det` 都是地图特有（s1 红 gen160 训练图 own-noise 0.08、换图 1.27/1.45）。
2. ⛔⛔ **「时代」叙事已删除**：逐色闸（tau≥0.6 三图全过 且 Spearman≥0.5 三图全过）在**加密采样 + 独立谱系**下复测，
   **四条轨迹全部 FAIL**（s1红 tau0.18 · s1蓝 0.48 · s2红 0.45 · s2蓝 0.35）。
   ⭐⭐ **「蓝方特殊」是 6 个采样点的运气**：s1 蓝 6 点 tau 0.73/1.00/0.73 → 11 点 0.60/0.59/0.48；
   s1 红训练图 Spearman 6 点 0.83 → 11 点 0.12。⇒ **六个冠军上的秩统计量不是证据（本轮它骗了我两次，方向相反）**。
   ⭐ s2 不复现颜色分裂（那边红方在进步）⇒ 真正重复的是**形状**：每条谱系恰好一方进步、另一方不进步（⚠ n=2）。
   ⚠ 未排除的替代解释：无接触格子分布不均（蓝 15–24 格 vs 红 3–4 格）⇒「次序不迁移」与「离图量得少」还没分开；
   修法是**离图加大每格场次**，⛔ 不是降门槛。
   ⇒ 还站得住的只有方法结论：**进步要逐色读；只有粗趋势曾看似跨图稳，逐代细次序四条轨迹一条都没迁移过。**
⭐⭐ **chunk 2b（推断血统，`npm run lineage`）也做完了**：⛔ trainer 不记录亲缘 ⇒ 全部从基因组推断（不跑仿真）。
四条轨迹一致：完全保留 11–20%、**「换人」= 0**（最大一步只有中位数 ~2.5 倍，第一代后没有尾巴）。
⭐ 加了 **lag 曲线**诊断把「零换人」的两种成因分开（平=种群收敛 / √lag=随机游走 / 线性=定向行军），
实测 **4.3–4.6×** ⇒ **受约束的随机游走**：持续在动、但被关在有界区域里。
⚠ 心智模型纠正：一次突变 RMS 0.0071，相邻冠军中位步长 0.063（九倍）⇒ **相邻代冠军不是亲子关系**。
⭐⭐⭐ **最该记住的一条**：**lurk 角色 72 代锁定、守 227 代，而冠军基因组在任何地方都没有不连续**
⇒ **稳定行为角色跨越了可观的基因改变而存在**（⛔ 描述不是机制）。⇒「谁改变了这支队」= **没有某一个谁**。
**下一步**：chunk 3 = 观战侧展示面（可画的骨架已就位）。G2 小尾巴（s2 蓝「只游荡不开火」）仍未做。
