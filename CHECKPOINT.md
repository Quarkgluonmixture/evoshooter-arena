...
⭐⭐⭐ **pair 也走完了三层，而结论是一次当天的自我更正**（`lurkswap --focus i --partner j`，三臂 + placebo，
外加**四条独立谱系**的复现集 d2s-pop48 / d2s-eval15 @99）：
⛔ 我一度把「C 臂里搭档仍留在同一批 agent 身上」读成「跟身份走」、写下「第一个住在身份里的形状」——**已撤回**。
⭐⭐ **原因 = 本轮最值钱的一条（坑 #37）：两条臂各自只拆掉一半配对，所以「它还在」正是「温和退化」的预测；
只有「搬家到现在持有那个标签的**别的** agent」才算归因。** ⚠ 留存型证据又干净又稳（60% ± 2.5pp、六臂一致），比真证据还漂亮。
**按搬家规则重判（数字没改，改的是推理）**：s1 蓝 0-1 / s2 蓝 0-1 **跟出生位走 ✅**（C 臂 by spawn 87–92% / 68–89%，
确实搬到了接手位置的身体上）；**s1 红 1-3 = 未确立**（唯一搬家证据是 B 臂 k=2 的 69%，另两次直接打散）；
s2 红 与 eval15-s1 蓝 **placebo 也打没 ⇒ 归因不了**；复现集四个冠军**只有留存没有搬家 ⇒ 归因不了**。
⚠ 复现集里过线的五个**全是相邻 4m**，两个非相邻的都没过 1.5× ⇒「跟身份走」没拿到新的检验对象。
⇒ **诚实结论：九个有搭档的冠军里只有两个能正向归因，且都归给出生位置；没有一个在搬家证据上跟身份走。**
⇒ 于是 lurk（角色跟出生位）与 pair（能判的都跟出生位）**指向同一件事：此处一切可归因的结构都是几何的**
⭐ 对 SUBSTRATE 的 Club/Player-block 仍是降温读数。
**下一步（各需预注册，⛔ 未跑）**：① **第四臂 = 把整份配对（出生位 + one-hot）一起搬走**，让它一半都不剩；
...
| `npm run lurkswap -- <run> --gen G --colour R\|B [--focus i --partner j]` | ⭐ **形状跟身份走还是跟出生点走**（VISION §12.1 层③）：换 one-hot / 换出生点 / placebo 三臂，按 carrier 与 spawn 双标签归因 | ⛔ 两个标签必须各维护一张表 —— 按 agent 下标算会把同一件事印两遍（坑 #26）；⛔ 只动被观测队；⚠ 两臂不一致 = 归因不了，别挑合心意的那条；⛔⛔ **只有「搬家到标签持有者」算归因，「还在原处」不算**（坑 #37） |
