...
**当前（2026-09-13 18:00）：⛔ 人工闸门 —— D2 的下一根杠杆需要用户定。**

链条（证据都在 Phase D2 进度）：世界付钱（memdemand）→ 合法电台够用、拿回 91–93%（radiodemand）→ 架构借口不成立（E1 probe）→
随机突变台阶探针分辨率不够（commstep）→ 完美说话者下 40 代没学会听、**120 代 2 条血统里 1 条学会**（uptake）。
⇒ 听是学得会的，但慢且看血统；真实 D2 要两边同时从零长出来。

| 选项 | VISION | 代价 / 问题 |
|---|---|---|
| **1. 只加预算，真实电台长跑** | 零冲突 | 本机 120 代 ≈ 27 分钟 / seed 且会被内存杀；几百代量级，结果为空也不构成结论 |
| 2. 脚手架 → 撤除 | **撞 §7.1 / §1.1**：长出来的是我们的词表 ⇒ 按 §15 先改 VISION | 只有用户能定 |
| 3. 搜索效率杠杆（种群 / comm 通路变异 / 说听协同变异） | 不冲突 | ROADMAP 无此 phase，是新研究方向 + 算力承诺 |

推荐：**1**（有预算上限、可续跑；先解决本机内存或换机器）→ 1 仍为空再开 **3** 的 phase；**2 不做**，除非用户先改 VISION §7.1。
