...
**当前（2026-09-13 15:47）：D2 的 search 侧 —— 先问「进化有没有台阶可走」。**

D2c 已答（见 Phase D2 进度）：一条合法的一符号电台就拿回 93% 的共享接触价值 ⇒ 世界有需求、通道够用，
E1 probe 又拿掉了架构借口 ⇒ 剩下的是**搜索 / 选择**。
VISION 对照：§7.1 要求语义「由队友通过共同训练学会解释」· §10 禁止给设计者喜欢的战术发糖 · §14.1 要先写清「缺腿还是没选」——
现在是**没选** ⇒ ⛔ 不加通信奖励、⛔ 不预设符号语义、⛔ 不改电台。
⇒ 下一刀的**候选探针（未做）**：通信是双边协调，只改 speaker 或只改 listener 的单边突变可能**永远不被选择**（没有台阶）。
量法：拿现有冠军，只突变 comm 输出行 / 只突变 `mate*.comm*` 输入列 / 两边一起，看 team fitness 变化的分布。
单边收益全 ≤ 0、双边偶有正收益 ⇒「台阶缺失」是具体机制，再对照 VISION 选杠杆。预测先冻结再跑。
