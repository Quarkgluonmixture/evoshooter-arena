...
- ✅ **pair 的干预跑完 + 复现 + ⛔ 自我更正（VISION §12.1 层③）**（2026-09-15）：
  `lurkswap --focus i --partner j` 把三臂对准一对搭档（交换与 placebo 都在这对之外）。**先记更正**：我一度把
  「C 臂里搭档仍在同一批 agent 身上」读成「跟身份走」并写下「第一个住在身份里的形状」——**当天撤回**：
  ⭐⭐ **两条臂各自只拆一半配对，所以「它还在」正是「温和退化」的预测；只有「搬家到现在持有那个标签的别的 agent」才是归因**（坑 #37）。
  **按搬家规则重判（数字未改）**：s1 蓝 0-1 与 s2 蓝 0-1 **跟出生位走 ✅**（C 臂 by spawn 87/92% 与 68/76/89%，
  确实搬到了接手位置的身体上）；**s1 红 1-3 = 未确立**（唯一搬家证据是 B 臂 k=2 的 69%，k=0/k=4 直接打散）；
  s2 红 0-2 与 eval15-s1 蓝 = **placebo 也打没 ⇒ 归因不了**；复现集（d2s-pop48 / d2s-eval15 四条**独立谱系** @99）
  四个冠军**只有留存没有搬家 ⇒ 归因不了**。⚠ 复现集里过线的五个**全是相邻（4m）**，两个非相邻的都没过 1.5×
  ⇒ 「跟身份走」没拿到新的检验对象。
  ⇒ **诚实结论：九个有搭档的冠军里只有两个能正向归因，且都归给出生位置；没有任何一个在搬家证据上跟身份走。**
  ⭐ 下一步（各需预注册，⛔ 未跑）：① 把**整份配对（出生位 + one-hot）一起搬**的第四臂；
  ② 两臂都没动的按 slot 挂的通道 = **感知噪声键**（`viewer.slot*31+target.slot`；⚠ 队友排序按距离不按 slot，不在其中）。
