...
**当前（2026-09-13 16:47）：D2 listener 侧 —— 先排除预算：同一个完美报点的脚手架，训 120 代学不学得会听？**

链条（证据都在 Phase D2 进度）：世界付钱（memdemand）→ 合法电台够用，拿回 91–93%（radiodemand）→ 架构借口不成立（E1 probe）→
随机突变台阶探针分辨率不够（commstep）→ **说话者完美时 40 代仍没学会听（uptake）⇒ 堵点在 listener 侧**。
VISION 对照：§14.1 一次一根杠杆 ⇒ 先试**只加代数**（不改世界、不改大脑）；§8 记忆是底座，但 radiodemand 的手写听者**不需要记忆**就拿到 91% ⇒ 记忆排最后；
§7.1 / §10 ⇒ ⛔ 不加通信奖励、⛔ 不预设语义、⛔ 脚手架不进 ship 的训练路径。
读数改成「同臂所有 run 的另一色冠军都打」（避开 98% / 2% 天花板），并用它重读 40 代基线。预测冻结在 `runs/d2-uptake120-predictions.txt`。
学得会 ⇒ 是搜索效率 / 预算问题，杠杆留在 search 侧；学不会 ⇒ 表征 / 可学性问题（听者要靠一个稀疏符号**门控**跟哪个点位向量），再对照 VISION 选容量或输入形式。
