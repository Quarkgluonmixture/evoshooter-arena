...
- ⏳ **Probe 1b**：沿 s2 蓝血统读 g139–159 留没留住（`--observe d2-long-s2:B`，预测 `runs/d2s-probe1b-predictions.txt`）；之后 Probe 1c 量选择噪声，再选杠杆。
...
**当前（2026-09-13）：Phase D2s（radio search efficiency）—— Probe 0 / 0b 完成（内容读数 = radiouse `replay`），Probe 1 待冻结。**

- 由来：D2 选项 1（只加预算 300 代）读完，P1、P2 成立、无趋势（见 Phase D2 进度）⇒ 按用户认可的顺序开 D2s。
- 下一步：d2-long 的 replay 描述（`runs/d2s-probe0b-long.txt`）读完后，冻结 Probe 1 的预测（找到过没有 / 找到了留没留住），再按答案选第一根搜索杠杆。
- ⛔ 选项 2（脚手架 → 撤除）仍要用户先改 VISION §7.1。
