...
- ⏳ **选项 1 进行中（用户 2026-09-13 选定：真实电台只加预算）**：`runs/d2-long-s{1,2}`，300 代（上限）。
  读数 `npm run radiouse`（只动被测队）在 d2-radio 对照组上改了三版，**对照门槛 P3'' 没过** ⇒ 长跑将逐冠军报告，不按 Δspeaker 门槛判。
- ⭐ **更正 D2b 的接收端结论（2026-09-13）**：「off 与 shuffled 伤害一样 ⇒ 满音量噪声」只对了发送端。
  ...
  D2b 两队一起消融、固定步长取池，看不见单队依赖。**发送端 MI 0–1% 仍成立**（只对 5 个候选指称）。
...
**当前（2026-09-13 21:04）：D2 选项 1（用户 2026-09-13 选定）—— 真实电台只加预算，300 代 × 2 seed 在跑。**

- 运行：`runs/d2-long-s{1,2}`，配置 = d2-radio，`--snap-every 10`（本机会以内存不足杀长任务，同命令重跑即续上）。
  上限 300 代，⛔ 延长需要重新登记预测。
- 预测：`runs/d2-long-predictions.txt`（含两段 addendum，都写在任何长跑被读之前）。
- 读数：`npm run radiouse -- runs/d2-long-s1.json runs/d2-long-s2.json --gens 39,149,299`（接收端）+ `npm run radio -- <run> --gen G`（发送端）。
- ⚠ 对照门槛 P3'' 没过（见 Phase D2 进度）⇒ 长跑**逐冠军报告**，不按 Δspeaker 门槛判；已知读数缺陷：comm0 / comm1 各自独立抽（TODO）。
- 下一步按用户认可的顺序：结果为空且无趋势 ⇒ 开**搜索效率** phase（选项 3），probe-first；
  选项 2（脚手架 → 撤除）仍撞 VISION §7.1 / §1.1，⛔ 需要用户先改 VISION。
