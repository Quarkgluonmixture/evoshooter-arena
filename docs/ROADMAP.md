...
**当前（2026-09-13 22:07）：Phase D2s（radio search efficiency）—— Probe 0：先修 radiouse 的尺子。**

- 由来：D2 选项 1（只加预算 300 代）读完，P1、P2 成立、无趋势（见 Phase D2 进度）⇒ 按用户认可的顺序开 D2s。
- Probe 0：comm0 / comm1 联合抽样（TODO）+ 电台静音训练的零点对照；门槛写进新的 `runs/*-predictions.txt` 再跑。
- 之后 Probe 1（找到 vs 留住），再按答案选第一根搜索杠杆。⛔ 选项 2（脚手架 → 撤除）仍要用户先改 VISION §7.1。
