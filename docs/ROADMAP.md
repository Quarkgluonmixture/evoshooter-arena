...
### ⛔⛔ 适用范围（2026-09-15 收口检验，先读这条再读下面任何一条 lurk 结论）

**lurk 角色不可跨图搬运。** 三张图的**出生点与点位逐位相同、只有掩体不同**，而干预已证明角色跟出生位置走 ——
即便如此：s1 红 @299 在**图 11 上还在**（0 号 60%、2.20× 零假设），在**图 23 上整个塌掉**（0 号 **0%**、孤立跑到 2 号、1.10× = 随机）。
反方向也成立：**s2 红在训练图上没有这个形状，换到图 11 就长出来一个**（0 号 64%、2.76×、32 次 episode）。
⇒ 这个角色是 **(基因组, 出生位, 这张图)** 的联合性质；⛔ **下面每一条 lurk 结论（含「守了 227 代」）都只在训练图上成立**。
⛔「这支队有个 lurker」不是这支队的性质。detector 的对照/零假设/birth/干预机制**都没被推翻**，只是**都跑在一张图上**。
...
## Phase E2b — 供给侧：世界为「混合」付钱，底座生得出混合吗？（2026-09-19，probe-only，已结案）

**Question**：E2 的 demand 表是本仓四条 demand 探针里**唯一读正**的一条（守方均衡混合 68/32，比任何
单一合法打法的最坏情况高 14.5pp）。那就该反过来问一次：**进化收得到这 14.5pp 吗？**

⛔ 本 phase **不建造任何东西**，只量一件事，然后把结论钉在纸面上。

### 仪器

`npm run mixing`（预注册 `predictions/e2b-mixing-predictions.txt` + 三段 addendum）。
计划 = t=T 时该队五个 slot 各自最近的点位（5 元组）；熵只在单个 (row, arm, map, role) 格内算。
三条臂：own dice（32 个 match seed）· opponents（对方 hof 跨史 6 个大脑）· maps。
闸 = `ctl fixed 3/2` 0.000 且 `ctl alternating` 1.000（坑 #42：检测性对照必须构造上确定）。

### 进度（2026-09-19）：**可以反应，不能不可预测**

- ⭐ **arm A 在每一个受保护格子上都是 0.000 bit**（2 条谱系 × 2 色 × 2 图 × 攻防）。
  同一个对手下，32 场的计划逐字相同。机制在源码里：`World` 的 rng 只滚命中骰子，
  感知抖动是纯哈希，出生点固定 ⇒ **玩家没有私有骰子**。
- ⛔ **arm B 是谱系性质，不是世界性质**：三个冠军读恰好 0，`d2-long-s2:R@299` 读 1.79 / 1.46。
  预测 P3（>0 且 <0.5）**两个方向都错**。
- ⭐⭐ **t=3 s 时全部为 0**；s2:R 的对手依赖 t=4 s 才出现、t=6 s 长到 1.79，而它首枪在 5.7 s
  ⇒ 是**接敌前**的合法条件化。反向：s1:R 在图 11 上 t=12/20/30 s 仍读 0.000（alive5 75–91%、首枪 4.0 s）。
- ⭐ **对照给出另一半**：`ctl alternating` 在同一世界同一仪器上读 **1.000**
  ⇒ **世界装得下混合的计划，只是不会为「每回合输入都相同」的策略生出一个。**

⇒ **判决：世界挂 0.904 bit，进化收到 0.000 —— 不是没学会，是底座缺这条自由度**（VISION §14.1）。
本仓第一次把一条 demand 读数判成**收不到**而非**没找到**。

### ⛔ 这条结论不授权的事

⛔ 不授权「加一个私有骰子 / 随机化出生点 / 开多回合历史」中的任何一个 —— 每一个都是**独立的承重杠杆**，
都要先过 VISION §15 的七问，并且 **E2 自己的读数已经警告**：混合只对**会适应的**对手兑现。
⇒ 真要往下走，下一步是 probe 而不是施工：**加了自由度之后，世界还付不付这 14.5pp、进化收不收。**
⚠ 另外三条随时可能被误读的边界：只量了**一个**统计量（空间承诺）· 只在两张图两条谱系末代上 ·
⛔ 0 bit 不说明冠军弱（`exploit` 四个目标只打穿一个）。

---

## Phase E2c — 私有骰子：把 E2b 判缺的那条腿补上，然后看进化捡不捡（2026-09-20，已结案）

**用户 2026-09-19 在 E2b 的闸门上选定**三个候选里的**私有随机输入通道**（另两个：随机化出生点 ——
⛔ 会拆掉 09-15 唯一跨图成立的那层「结构由出生位置扛着」；多回合历史 —— 撞 E7 合同）。

### Build（已 ship，默认关）

`SimConfig.privateDieDim`：每人每回合 k 个 [-1, 1) 的数，取自 (team, slot, match seed) 的**纯哈希**
（⛔ 不消费战斗 rng，否则带骰子的 run 永远无法与不带的比较），整回合不变，**只进自己的 observation**。
appended 在观测向量**最后**，所以它前面每个下标的含义不随通道开关改变。
`privateDieDim` 进 `BRAIN_ONLY_FIELDS`（机械验证：改它不改手写 bot 的那场比赛，逐位）
⇒ **`yardstick` 是这次 A/B 唯一合法的跨边界尺子**（genome 5684→5724，坑 #27 让同 seed 配对作废）。

⭐ **镜像测试这一关值得单独记**：它断言红蓝逐位相同，而诚实的私有骰子**必须**打破它
（逐位镜像若仍成立 = 两队掷出同一个数 = 谁都没有私有的东西），而坑 #17 又禁止「关掉通道再测」。
⇒ 改成**带骰子跑 + 断言不同的字段恰好是骰子且是全部骰子**；side fairness 降回分布式断言
（400 seed，两色各自 |mean| < 0.05、sd ∈ [0.54, 0.61]）。隐私由 `A1-P27` 双向守（别人的骰子动不了我、
我自己的恰好动 `self.die*`），它**自己把通道打开**再测，否则在默认关的配置下是空过。

### 进度（2026-09-20）：捡起来了，是它造成的，但长出来的是「每人各自掷」

- ⭐ **捡起来了**：gen 0 → gen 299 八个守方格子**七个上升**（均值 0.40 → 1.66 bit），会动的 slot 从 ≤1 到 1–4。
  ⚠ 零假设**不是零** —— 只训 2 代的随机网络就读 1.703 bit（随机权重透传骰子）⇒ 必须比同一条 run 的 gen 0，
  这条在读长跑之前就换掉了（预注册 addendum 1）。
- ⭐⭐ **干预层干净**：`--freeze-die`（每场用第 0 场那轮的**合法**骰值；⛔ 不清零，清零是 OOD 坑 #24）
  ⇒ **八格全部塌回 0.000 / 1 plan** ⇒ 变化**由骰子造成**，不是末代权重更敏感。
- ⭐ **但不是队级混合**：计划集合按独立逐 slot 翻转**几乎精确可分解**（观测 14/7/6/5 vs 独立模型 13.1/7.9/6.9/4.1）。
  ⇒ **五颗私有骰子没有共同抽样，只能各掷各的；队级混合需要一次共同的抽，而共同的抽只能靠说出来。**
  ⇒ ⭐⭐ **与 D2 / G3 接上了**：要队级的不可预测，先得有**带内容的通信** —— 这条谱系 300 代从没长出来的那个。
- ⛔ **没有证据说它值钱**：`yardstick` 无骰子臂 85.5% vs 有骰子臂 79%（⚠ 单格 ±21pp、2 seed，只读方向）。
- **坑 #40**：存在性跨图（八格全 > 0），量级不跨图（s2 图 7 读 1.87–2.10、图 11 只有 0.76–0.86）。

⇒ **判决**：E2b 的「同一对手下恒为 0.000」在有骰子的臂上**不再成立**，且因果成立。
⛔ 但正确的结论**不是**「私有骰子有用」，也**不是**「没用」，而是：
**这个预算、这个对手分布下，不可预测性既不受惩罚也不受奖励。**
⇒ 通道**保留、默认关**（VISION §14.2：回退实现，不回退现象）。
⇒ **下一根杠杆指向 E6 对手生态**：要有人来收这 14.5pp，得先有一个会 best-response 的对手。
...
- ❌ **crossfire detector 第一版不合格，已关闭**（2026-09-14）：定义「敌人处的夹角 ≥ 60°、且 ≥ 2 名队友看得见」。
  precision 两次都没过 —— 先是正样本选错（分守两个点位的人根本不会同时看见同一个敌人：中位夹角 5° < 挤在一起的 11°）；
  换成手写守位 bot（`PostHolderPolicy`，同点位两侧）后 flanking 27% vs bunched 20%（只有 1.35×，且负样本 > 15% 的门槛）。
  ⭐ 把阈值提到 110° 反而**反超**（bunched 14% > flanking 9%）⇒ 根因是**「敌人处的夹角」把侧翼包夹和敌人贴脸混在一起**。
  ⇒ 按冻结规则 **不读冠军**；修法（两名观察者都要离敌人足够远）记进 TODO。⚠ 冠军行与对照同表打印过，重做时的预期已被污染。
- ✅ **crossfire 第二版（2026-09-15）：加上「两名观察者都离敌人 ≥ 6 m」之后 precision 过了**（flanking 11% vs bunched **0%**，两行分母 > 1900）。
  读冠军（描述，⚠ 预期被污染，见 `runs/g2-crossfire2-predictions.txt` 开头）：d2-long @299 四个冠军**一次都没有**形成这个形状，
  中位夹角 3–22°（门槛 60°）。⇒ **它们抱团打**，与 E2 的均衡读数（这个世界付钱给抱团）一致；⛔ 不等于「学不会交叉火力」。
- ❌ **rotate（pressure → switch）第一版关闭，冠军未读**（2026-09-15）：定义 = 守方从「离冷点更近」在 3 秒内变成「离热点更近」。
  两轮 precision：第一轮对照与零假设都坏（守点 bot 在点位被占后回抢 ⇒ 38%，不是负对照；手写攻方每场打同一点 ⇒ 零假设倍数恒 1.00）；
  修好后（固定守位 bot 当负对照 **0%** ✓、攻法逐场轮换）仍不过 —— ⭐ **对压力无视的定时轮转读 39%，而真正按压力汇聚的反应式只读 5%（低于自己的零假设，0.45×）**
  ⇒ 这个定义量的是「多久跨一次中线」，不是「对压力的响应」：3 秒窗口在这张图上跨不完半场。
  ⇒ 按冻结规则关闭，⛔ 不再调阈值；重做要新定义 + 新预测（时间尺度从「跨半场要多久」推，不是拍一个 3 秒）。

- ❌ **man-disadvantage tempo（clutch）第一版关闭，冠军未读**（2026-09-15）：定义 = 逐 tick 按 `alive(我) − alive(敌)` 标
  DOWN / EVEN / UP，tempo = 速度 m/s，统计量**逐玩家配对**的 DOWN 均速 − EVEN 均速，再与零假设做**差中差**（mean ± SE）。
  两处 harness 修复都没动冻结门槛（供体改成**换 seed 且不短于本场**；「每状态至少 2 秒」按实测改成 8 tick ——
  逐玩家 DOWN 时长 p50 只有 4 tick，对撞行**没有一个玩家**到得了 30 tick）。
  ⭐ 关键读数：**正对照（少人就原地不动，按构造 100% 因果）raw −4.75 m/s，DiD 只剩 −0.48 —— 零假设吃掉 88%**；
  负对照的供体与本场在 **95% 的 tick、94% 的 DOWN tick** 上一致 ⇒ 它那个干净的 +0.06 ± 0.02 是**空过**。
  ⇒ 根因是**这个世界里人数差几乎是回合时钟的函数**（第一个人倒下在 tick 68 ± 15；~26 个秒级 bin 里只有 2–4 个同时装得下两种状态），
  跨场置换因此不是零假设（GOTCHAS #35）。⛔ 这**不是**「冠军没有 tempo 响应」——P2 未被回答。
  ⭐ 重做要**场内反事实**：在 tick T 分叉、只在一支上抹掉一个人，与**未分叉的自己**在同一时钟下比 —— 那属于 G4c 的干预，
  是新的预注册，⛔ 不是在这里调门槛。
- ⛔ **关闭的 detector 默认不打印冠军行**（2026-09-15，rotate / tempo 均已加闸）：`--reopen rotate,tempo` 才打开，
  且只在配**新预注册**时才合法。理由是量过代价的 —— crossfire v1 失败那次冠军行与对照同表打印，污染了重做时的先验。

- ✅ **lurk / 孤立路径 detector 建成，precision 4/4 通过，⭐ G2 的第一个非空读数**（2026-09-15）：
  定义 = 存活、且至少 2 名队友还活着、且 **12 m 内没有任何活着的队友**；连续 **2.0 s** 以上算一次 episode。
  ⭐ 两个数都从世界量出来：12 m 直接沿用 trade detector 的「同一场打斗」半径；2 s 来自实测单挑长度（p50 1.40 s / p75 1.93 s）。
  零假设是**场内 slot 标签置换**（200 次重抽），⭐ 这正是选空间形状的理由 —— 它不碰时钟（对比 tempo 的失败，坑 #35）。
  对照修了一处：role-negative 的 slot→post 固定会**自己带角色**（坑 #36），逐场轮转后 0.91× ✓。
  **冠军（仅描述）**：s1 红 **0 号占自己可计 tick 的 47%**（其余四个 slot 0%），24 场 24 次 episode，中位 6.1 s，
  集中度 **3.17×** 零假设 —— 与手写 lurker 对照的 3.08× 同级；episode 内**跑 2.82 m/s、打出 390 伤害、4 个人头**（不是卡住的身体）。
  s2 蓝 1 号 20%、11 次、中位 8.0 s、2.01×，但 episode 内**完全不开火** ⇒ 孤立游荡，不是包抄。
  ⚠ s1 蓝 / s2 红只有 2 次 / 1 次 episode，角色比值**不可读**（零假设本身坐在 80–89%）⇒ 报「没有形状」，⛔ 不报「没有 lurker」。
  ⇒ 按 claim ladder 只到 **CANDIDATE**。还差 **birth**（扫世代找它什么时候出现）与 **intervention**（slot one-hot 对调，
  看它跟 slot 走还是跟身体走 —— 五个身体共用一个脑子，角色只可能住在 slot 输入里），⛔ 两者各需新的预注册。

- ✅ **lurk 的 birth 扫完，VISION §12.1 层① 完成**（2026-09-15）：同一 detector（定义/门槛**未动**）加 `--gens` 与
  `--opponent-gen`，两条臂 = 固定对手 @299 / 匹配代共演化。**slot 0 的角色在第 72 代锁定**（占自己可计 tick 的 62–64%、
  集中度 3.22× 零假设、24 次 episode），此后**每个采样代、两条臂**都在 slot 0，直到 299 代 —— **227 代**；
  前兆出现在 60–68 代但**身份在闪**（64 代又没了）⇒ 按冻结分支算 drift 不算 birth。
  ⚠ 第 0 代的 13% 是**预注册过的退化混淆**（episode 内 1.25 m/s、全行 ≤ 26 伤害 = 谁都不接敌，坑 #13），⛔ 不能当形状读。
  ⭐ 两臂唯一的分歧本身是结论：150–200 代在匹配代臂上会派**第二个**人去游猎（集中度掉到 1.75–2.38×），
  对固定的 299 代对手则仍只派一个 ⇒ **派几个是对手依赖的，派谁不是**。
  ⚠ 每代只有一个冠军基因组 ⇒ 「72 代锁定」说的是**冠军线**不是种群。⛔ 仍只到层①，层③ 的 slot 对调需单独预注册。

- ✅ **lurk 的干预（VISION §12.1 层③）跑完 —— 角色跟出生位置走，不跟身份输入走**（2026-09-15，`npm run lurkswap`）：
  三臂 = 对照 / 换 one-hot（身体不动）/ 换出生点（身份不动），只动被观测队，detector 定义未改。
  **两条臂一致**：读着 one-hot 0 的身体只扛 6–8% 的孤立，挂在**出生位置**上的那个扛 29–54%。
  ⭐ 事后补的 **placebo**（换两个非 lurker 的身份）**完全不掉**（占比 10% = 对照，slot0 保住 90%）
  ⇒ 「越界编辑会打散一切」这条反驳被答掉：打散它的是动了 0 号**自己那一对**。
  ⭐⭐ 但也不是纯几何：两条臂的总孤立都**腰斩**（10% → 2–8%）⇒ **形状要的是训过的那一对 (spawn 0, one-hot 0)**。
  ⇒ **「lurker」= 从那个侧翼出生的身体；one-hot 是配对的一半，不是可搬运的身份。**
  ⭐ 对 SUBSTRATE 的 Club / Player-block 方向是**降温**读数：**此处的 player block 还不是能转会的东西**
  （E5/E6 真要做 transfer 时，这是第一条要重新测的）。
  ⚠ 只做了一个冠军（s1 红 @299），跨 k 很不均匀（by-spawn：B 臂 0–86%、C 臂 15–100%），± SE 是对四个 k 求的。

- ✅ **pair detector 建成，precision 3/3 过 —— 四个冠军都有稳定互选搭档，⭐ 两个「跳过邻居」**（2026-09-15）：
  距离阈值在这里饱和（32% 的 pair-tick 在 4m 内），所以量的是**谁是谁的最近**这个**关系**；
  互选 strength = min(P(i→j), P(j→i))，零假设仍是场内 slot 标签置换。
  负对照修了两处（⛔ 都不是定义/门槛）：**循环轮转保住了「两个 slot 的差」这个不变量**⇒ 60% 假阳性；
  同一 seed 的两种攻守共用置换 ⇒ 多出零假设没有的对齐。改成**逐场逐攻守的随机置换**后 0.94× ✓。
  **冠军**：s1 红 1-3 = 64%（2.14×，出生间距 **8m 跳过中间人**）· s1 蓝 0-1 = 91%（2.55×，4m）·
  s2 红 0-2 = 56%（1.72×，**8m 跳过**）· s2 蓝 0-1 = 71%（2.05×，4m）。⚠ 邻接是**量出来的**（出生点是直线、每 4m 一个，两队都核过）。
  ⭐⭐ **这是第一个出生几何解释不了的形状** —— 上一轮干预刚证明 lurk 角色跟位置走，那就预测搭档该是相邻出生点，
  而两个红冠军跳过了中间那个人。⇒ CANDIDATE；下一步把 lurkswap 三臂套到 pair 统计上（需单独预注册）。
  ⛔ 命名纪律：不叫 "pair coordination"（暗含共同意图），叫**「反复出现的互选最近邻对」**。

- ✅ **pair 的干预 + 复现 + 两次自我更正，结论：能归因的搭档全都归给出生位置**（2026-09-15，`lurkswap`）：
  三臂 + placebo + **D 臂（整份配对一起搬，预注册的自我检验）**，另跑**四条独立谱系**的复现集（d2s-pop48 / d2s-eval15 @99）。
  **两次更正都记在 `LOG.md`**：① ⭐⭐ **「它还在」不是归因，「搬家到标签持有者」才是**（坑 #37）——
  两条臂各自只拆一半配对，留存正是温和退化的预测；② ⛔ **arm C 有真 bug**（helper 默认参数把换点钉死在 slot 0，
  `--focus≠0` 的**五次**运行换错了身体；落地断言拿自己 re-derive 的地图比对、容差 8m > 间距 4m，恒真）——坑 #38。
  **修完重跑九个冠军**：**五个正向归因，全部归给出生位置**（C 臂 by spawn 38–78% vs by carrier 3–9%）；
  三个归因不了（两个被自己的 placebo 判的）；**s1 红 1-3 不是位置，但身份证据弱且散（28% ± 20.8）⇒ 未确立**。
  ⭐ **D 臂如预期**（s1 红 35/42/59% vs 对照 64%、eval15-s1 红 61/68/52% vs 65%）⇒ 位置+身份一起换时搭档跟着走
  ⇒ 两臂都没动的按 slot 挂的通道（**感知噪声键**）什么都没扛 ⇒ **双标签分解成立**。
  ⇒ **lurk（角色跟出生位）与 pair（能判的全跟出生位）指向同一件事：此处一切可归因的结构都是几何的。**
  ⭐ 对 SUBSTRATE 的 Club/Player-block 仍是降温读数：**这里的 player block 还不是能转会的东西**。

- ✅ **最后一条线索收掉：s1 红那对确实跟身份走，但只属于 299 代那个基因组**（2026-09-15）：
  加了两处仪器（定义未动）——**搬家统计的基线**（「搬到 k」要对照「(k, 搭档) 本来就有多少」）与**对称检验**（改动另一半的身份）。
  **对称检验通过**：把 one-hot 3 给谁，谁就和「读 one-hot 1 的身体」配对，三次全中且全高于基线
  （60% vs 2% · 73% vs 32% · 27% vs 2%），配合原方向六条臂 by-spawn 全 ≤ 2% ⇒ **299 代这一对跟身份走**。
  ⚠ 不对称是真限制：**锚（身份 1）只转得动相邻身体，搭档（身份 3）给谁都转得动** ⇒ 锚那一半仍要它的位置。
  ⛔ **但世代上不稳**：1-3 更早根本不存在（150 代 3-4、200 代 1-2、250 代 3-4，**都相邻**，
  且探索性地看**都跟位置走**）⇒ 搭档约每 50 代换人，**非相邻那一对是 299 代才发生的事**。
  ⭐⭐ **本轮结论 = 这个对比**：**位置型角色（lurk 0 号）稳了 227 代；身份型搭档只出现过一次、很晚、不稳定。**
  ⛔ 引用身份那条必须带「仅 299 代」+「锚/搭档不对称」。
...
- ✅ **chunk 1 = style space 建成（`npm run style`）**：逐代风格向量（⭐ 直接复用 `match.ts` 已有的 22 个 `METRIC_KEYS`，
  ⛔ 不新造特征）+ PCA 二维嵌入 + ⭐ **逐代噪声底**（每代在两个不相交的种子块上各测一次）。对手**固定**为另一色 @299
  （arm-A 教训：对手会动时「风格变了」和「对手变了」是同一个数）。
  ⭐⭐ **两道闸都是防我自己读错而加的**：① 不等间距扫描时大声警告（跨 50 代的 step 不能和跨 2 代的并排读）；
  ② ⭐ **噪声底是「重掷命中骰子」的底，不是「重跑世界」的底** —— 实测 `world.rng` 只用在命中判定，感知抖动是确定性哈希，
  地图/出生点固定 ⇒ **不接敌的冠军 own-noise = 0.00**，会把邻居的 step 撑得巨大 ⇒ 这类行标 `det` 不得读。
  实测收获：s1 红 gen 160 看似全扫描最大的分界线，其实 own-noise 只有 0.08（那个冠军几乎不接敌）——**没有这道闸就会被当头条报出去**。
  **结果**：四条轨迹都在最早一步动得最凶（年轻种群应然，sanity check 通过）；清过局部底的步数 3–7/14。
  ⛔ **P2 未确认**：5 代一格细扫显示 65 代之后几乎每步都清过底 = 连续 churn，**不是** gen 72 处的单一分界
  ⇒ style 对 lurk 的 birth 既不背书也不否定。
- ⛔⛔ **外部效度检验（同日）：「哪几代动了」是地图特有的** —— 同一网格在三张图上跑，一致率 42% / 83% / 43%，
  对应偶然水平 50% / 62% / 50% ⇒ **三对里两对不高于偶然**。唯一跨图成立的是「最早那一步动得最凶」。
  ⚠ 连 `det` 也是地图特有的：s1 红 gen 160 在训练图上 own-noise 0.08，换图是 1.27 / 1.45（**在自己长大的图上不接敌，换图就打**）。
  ⇒ **chunk 1 的每句风格结论都要带「在训练图上」**，⛔ **chunk 2 不得拿这条轨迹讲「时代」**。
  ⭐ 用词也纠正了：换地图不是「把噪声底做强」（地图/出生点/策略都确定，本就没多少采样噪声），它是**外部效度**检验。
- ✅ **chunk 2a = eras（`npm run eras`）**：⛔ 没造新矩阵 —— `crossplay` 已有 `--gens every:N` / `--maps` / 双色对打 /
  接触分母 / 非传递环，且 `crossplayFile.ts` 明令下游不得重算；新脚本只**消费** `rowMean/balanced/sight/cycles`。
  跑 `--gens every:60 --n 12 --maps 7,11,23`（每格 24 场）。
  **进步 Spearman(代数, rowMean)**：蓝 **0.94 / 0.94 / 0.94**（三张图全单调）· 红 0.83 / 0.43 / **0.03**（出了训练图就没了）。
  **跨图序稳定 Kendall tau（全 12 entrant）**：0.52 / 0.64 / 0.39 ⇒ **冻结的闸没过 ⇒ ⛔ 本轮不授权任何「时代」结论**。
  ⚠ 事后逐色看（⛔ 不算过闸）：蓝 0.73 / **1.00** / 0.73，红 0.47 / 0.20 / 0.47 ⇒ **失败不均匀：蓝的阶梯换图完好，红的是训练图产物**。
  **非传递环按图 0 → 1 → 22** ⇒ 离开训练图，非传递性爆炸（「大家在这儿都很平庸」的样子）。
  ⇒ ✅ 唯一被授权的是一条**方法结论**：**「进步」主张必须逐色做、且要在训练图之外复核**。
- ⛔⛔ **逐色闸复测（同日）：四条轨迹全部没过 ⇒ 「时代」叙事从 chunk 2b 删除**（`runs/g3-eras2-predictions.txt`）：
  采样加密到每 30 代（6 点→11 点）+ 独立谱系 s2 + 两色分开跑。结果 s1 红 FAIL（两半）· s1 蓝 FAIL（tau 0.48）·
  s2 红 FAIL（tau 0.45）· s2 蓝 FAIL（两半）。
  ⭐ **「蓝方特殊」是 6 个采样点撑出来的**：s1 蓝 6 点时 tau 0.73/1.00/0.73、11 点时 0.60/0.59/0.48；
  s1 红从另一侧撞同一堵墙（训练图 Spearman 6 点 0.83 → 11 点 0.12）。⇒ **六个冠军上的秩统计量不是证据**。
  ⭐ s2 **不复现颜色分裂**（那边是红方在进步）⇒「蓝方会进步」不是关于颜色的事实；真正重复的是**形状**：
  **每条谱系里恰好一方进步、另一方不进步**（像单边军备竞赛，⚠ n=2，是待检模式不是定律）。
  ⚠ **替代解释未排除**：丢掉的无接触格子分布不均（s1 蓝 15/150、s2 蓝 24/141 vs 红方 3–4/160）
  ⇒「次序不可迁移」与「离图后量得更少」尚未分开；修法是**离图加大每格场次**，⛔ 不是降门槛，需单独预注册。
- ✅ **chunk 2b = 推断血统（`npm run lineage`）**：⛔ trainer 不记录亲缘（hof 只有 `{gen,fitness,genome}`）⇒ 全部**从基因组推断**，
  一次仿真都不跑。四条轨迹一致：**完全保留 11–20%**、**3× 中位数的「换人」= 0**（一条 run 里最大一步只有中位数的 ~2.5 倍，
  第一代之后没有尾巴）。⭐ **但「零换人」有两种完全不同的成因**（一条线连续漂移 vs 种群收敛到看不出替换），
  冻结的设计分不开 ⇒ 加了**距离随 lag 的曲线**诊断：平=收敛云团、√lag(11.3×)=随机游走、线性(128×)=定向行军，
  **实测 4.3–4.6×** ⇒ **受约束的随机游走**（持续在动但被关在有界区域里）。
  ⚠ 顺带纠正心智模型：一次突变 RMS 0.0071，而相邻冠军中位步长 0.063（九倍、动 15% 权重）⇒ **相邻代冠军不是亲子**。
...
- ✅ **player identity cards**（2026-09-15，`detect --out` + `npm run cards`，⛔ 没有第三个 play loop）：
  ⭐ **设计被地图结论反过来约束** —— 卡片是最看起来笃定的界面，所以每条事实都逐图测、带 **TRAVELS/VARIES**，
  只有全图一致才平铺直叙；⛔ 卡上不出现任何角色名。**实测：三张图上「既稳定最孤立、又有稳定搭档」的玩家一个都没有**；
...
**当前（2026-09-20）：E2c 已结案 —— 腿补上了、被用上了、但这个对手分布不给它回报。下一根杠杆指向 E6。**

- ⭐⭐ **E2c（2026-09-20）**：私有骰子 ship（`privateDieDim`，默认关）。gen 0 → gen 299 八个格子七个上升，
  `--freeze-die` 使八格**全部塌回 0.000** ⇒ 因果成立。⛔ 但长出来的是**每人各自掷**（计划集合按独立逐 slot
  翻转几乎精确可分解），**不是队级混合** —— 队级混合需要一次共同的抽，而共同的抽只能靠**说出来**。
  ⭐⭐ 于是 E 线和 D2/G3 接上了：**要队级的不可预测，先得有带内容的通信。**
  `yardstick` 85.5% → 79%（只读方向）⇒ **不受惩罚也不受奖励**。逐条数字见 Phase E2c + LOG 2026-09-20。
- ⇒ **下一根承重杠杆：E6 对手生态**（要有人来收这 14.5pp，得先有会 best-response 的对手）。
  ⚠ E6 的前提「冠军很脆」在 40 代预算下没测到（见 E6 进度）⇒ 开工前仍要一个 probe。

<details><summary>2026-09-19 E2b 结案时的记录</summary>

**当时（2026-09-19）：E2b 已结案 —— 世界为混合付钱，底座生不出混合。下一根承重杠杆停在人工闸门。**

- ⭐⭐ **E2b（2026-09-19）**：`npm run mixing`。同一个对手下，四个冠军 × 两图 × 攻防的计划熵**恒为 0.000 bit**，
  而 E2 的均衡混合值 0.904 bit ⇒ **这 14.5pp 收不到，不是没学会**。对照 `ctl alternating` 在同一世界读 1.000
  ⇒ 世界装得下混合，只是不会为「每回合输入都相同」的策略生出一个。逐条数字见 Phase E2b 进度 + LOG 2026-09-19。
- ⛔ **E2b 不授权施工**：私有骰子 / 随机出生点 / 多回合历史各自是独立杠杆，都要先过 VISION §15 七问；
  而且混合只对**会适应的**对手兑现 ⇒ 往下走的第一步仍是 probe，不是 build。
- ⚠ **G3 全部条目已交付或已被明确回答**，G4（Discovery Feed）**本次明确不开**：它是把「新东西」浮出来的
  观战仪器，而 G2/G3 的证据说这条谱系里没有可发现的东西（电台 300 代零内容、身份层不跨图、风格空间
  认不出共演化的那一对）。⇒ 要开 G4，先得有一个值得被发现的世界。

</details>

<details><summary>2026-09-14 的当时记录（E 线四个人工闸门）</summary>

**当时（2026-09-14）：Programme E —— E1 无证据不动工，E 线在 E2（团队 prior），先问世界 demand。**
