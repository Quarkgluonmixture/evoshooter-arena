...
- ✅ **pair 的干预跑完（VISION §12.1 层③）—— ⭐⭐ 同一个冠军里两种形状分别住在两个地方**（2026-09-15）：
  `lurkswap --focus i --partner j` 把三臂对准一对搭档（交换与 placebo 都只在这对之外，⭐ placebo 这次在设计里就有）。
  **s1 红 1-3（8m）跟身份走**：C 臂 by carrier **60% ± 2.5pp** vs by spawn **1%**，六条臂 by-spawn 全 ≤ 3%；
  s1 蓝 0-1（4m）与 s2 蓝 0-1（4m）**跟位置走**（by spawn 76–81%，placebo 完好）；
  ⚠ s2 红 0-2（8m）**归因不了** —— 两臂都塌**且 placebo 也把它打没**（0/1/9%），正是预注册的 outcome D。
  **中途把「分界线是邻接」这个假说写进预注册再跑最后两个：s2 蓝 ✅、s2 红 ❌** ⇒ 假说只过一半，⛔ 不改轴名。
  ⭐⭐ **这是本程序第一个住在身份输入而不是几何里的形状**，而且与同一冠军的 lurk 结论**相反**（角色跟位置走）
  ⇒ 同一支队、同一个基因组、两种形状、两个载体，⛔ 排除了「这个冠军就是纯位置/纯身份」的偷懒解释。
  ⚠ **复现状态 n=1 干净 + n=1 归因不了**，⛔ 引用必须带上这两个数字；下一步是更多冠军，不是更强的词。
