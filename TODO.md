...
- [ ] `scripts/radio.ts` 的 `shuffled` / `alien` 用**固定步长**走消息池（`k + 7919`），不是随机抽。
      池子若有时间结构，步长游走可能仍带相关性 ⇒ 换成带 seed 的随机抽，并验一次结论不变。
- [ ] `objectiveProgress` 在角色互换后是攻/守**两种量的混合**（已写进 `TeamMetrics` 注释）。
      要按角色分别看的话得拆成两个指标，各带自己的分母。

## 观战 / 导播
- [ ] ⭐ **双点位的观战渲染**：`scene.ts` / `heatmap.ts` 只画 `map.zoneX/zoneZ`（= site 0）⇒
      `siteCount: 2` 下画面是错的。默认仍是单点位所以没发出去，但训练已经在双点位世界里跑了。
- [ ] ⭐ **kill feed**（谁打死谁，右上角滚动）—— 前置已就绪：`world.events` 已明确标成 **SPECTATOR CHANNEL**，
      观战器可以合法消费它；⛔ 不要从别处另造一套。
